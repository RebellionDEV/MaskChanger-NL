...
Cheat.RegisterCallback(
    ...
    function()
        ...
        if s == 0 then
            ...
            cache = masks:GetInt()
            EngineClient.ExecuteClientCmd(
                string.format(
                    'ent_fire !self addoutput "targetname facemask"; prop_dynamic_create %s; ent_setname %s; ent_fire %s disablecollision; ent_fire %s setparent facemask; ent_fire %s setparentattachment facemask',
                    filepath[number],
                    names[number],
                    names[number],
                    names[number],
